...
## ~~`await_async` is a no-op on headless and windowed backends~~ (resolved)
...
so pixels will not match GPU-rendered output (`:windowed` backend) exactly. Use
headless screenshots for catching layout regressions and verifying the
rendering pipeline; use windowed screenshots for pixel-perfect visual regression
against GPU output.
...
**What:** The headless and windowed backends spawn a renderer process via
