...
```elixir
alias Plushie.Event.{WidgetEvent, Key, Mouse, Touch, Ime, WindowEvent, Effect, SystemEvent}
```
...
Mouse area events use `%WidgetEvent{}` with `mouse_*` types:

<!-- test: events_mouse_area_enter_match_test, events_mouse_area_move_match_test -- keep this code block in sync with the test -->
```elixir
alias Plushie.Event.WidgetEvent

%WidgetEvent{type: :mouse_right_press, id: "canvas"}
%WidgetEvent{type: :mouse_enter, id: "tooltip-target"}
%WidgetEvent{type: :mouse_move, id: "drag-zone", data: %{x: x, y: y}}
%WidgetEvent{type: :mouse_scroll, id: "scroll-zone", data: %{delta_x: dx, delta_y: dy}}
```

Available types: `:mouse_right_press`, `:mouse_right_release`, `:mouse_middle_press`,
`:mouse_middle_release`, `:mouse_double_click`, `:mouse_enter`, `:mouse_exit`,
...
def update(model, %WidgetEvent{type: :mouse_enter, id: "hover_zone"}) do
  %{model | hovered: true}
end

def update(model, %WidgetEvent{type: :mouse_move, id: "canvas_area", data: %{x: x, y: y}}) do
  %{model | cursor: {x, y}}
end
```
...
the canvas node. Canvas events use `%WidgetEvent{}` with `canvas_*` types
and coordinate data in the `data` field.

<!-- test: events_canvas_press_match_test, events_canvas_move_match_test -- keep this code block in sync with the test -->
```elixir
alias Plushie.Event.WidgetEvent

%WidgetEvent{type: :canvas_press, id: "draw_area", data: %{x: x, y: y, button: :left}}
%WidgetEvent{type: :canvas_release, id: "draw_area", data: %{x: x, y: y, button: :left}}
%WidgetEvent{type: :canvas_move, id: "draw_area", data: %{x: x, y: y}}
%WidgetEvent{type: :canvas_scroll, id: "draw_area", data: %{x: x, y: y, delta_x: dx, delta_y: dy}}
```

The `button` field is an atom (`:left`, `:right`, `:middle`). The
`x`/`y` coordinates are relative to the canvas origin.

```elixir
alias Plushie.Event.WidgetEvent

def update(model, %WidgetEvent{type: :canvas_press, id: "draw_area", data: %{x: x, y: y, button: :left}}) do
  %{model | drawing: true, last_point: {x, y}}
end

def update(model, %WidgetEvent{type: :canvas_move, id: "draw_area", data: %{x: x, y: y}}) do
  if model.drawing do
...
events. These arrive as `%WidgetEvent{}` structs with `canvas_element_*` types.
...
%WidgetEvent{type: :sensor_resize, id: "content_area", data: %{width: w, height: h}}
```

```elixir
alias Plushie.Event.WidgetEvent

def update(model, %WidgetEvent{type: :sensor_resize, id: "content_area", data: %{width: w, height: h}}) do
...
use `%WidgetEvent{}` with `pane_*` types.

<!-- test: events_pane_resized_match_test, events_pane_clicked_match_test -- keep this code block in sync with the test -->
```elixir
alias Plushie.Event.WidgetEvent

%WidgetEvent{type: :pane_resized, id: "editor", data: %{split: split, ratio: ratio}}
%WidgetEvent{type: :pane_dragged, id: "editor", data: %{pane: source, target: target}}
%WidgetEvent{type: :pane_clicked, id: "editor", data: %{pane: pane}}
```

```elixir
alias Plushie.Event.WidgetEvent

def update(model, %WidgetEvent{type: :pane_resized, id: "editor", data: %{split: split, ratio: ratio}}) do
  put_in(model, [:splits, split], ratio)
end

def update(model, %WidgetEvent{type: :pane_clicked, id: "editor", data: %{pane: pane}}) do
