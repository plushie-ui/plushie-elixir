...
has its own struct under `Julep.Event.*`.

```elixir
alias Julep.Event.{Widget, Key, Mouse, Touch, Ime, Window, Effect, System, Sensor, MouseArea, Canvas, Pane}
```
...
```elixir
def update(model, %Julep.Event.Modifiers{} = mods) do
