...
```elixir
alias Plushie.Event.{WidgetEvent, Key, Mouse, Touch, Ime, Window, Effect, System, Sensor, MouseArea, Canvas, Pane}
```
...
events. These arrive as `%WidgetEvent{}` structs (not `%Canvas{}`). The
