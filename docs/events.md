...
matches. The `id` is the text editor's node ID. The `data` map contains
details about the matched binding (e.g. the action name).

```elixir
def update(model, {:key_binding, "editor", %{"action" => "save"}}) do
  {model, save_file(model)}
end

def update(model, {:key_binding, "editor", %{"action" => "format"}}) do
