...
|---|---|
| `Simulator` (in-process headless) | `Julep.Test.Backend.Headless` (cross-process via `julep_gui --headless`) |
...
| -- | `screenshot("name")` pixel screenshot (full backend only, no-op on sim/headless) |
...
|---|---|
| Tree storage and query | Julep's `Tree` struct in Rust, queried via JSONL protocol |
| Event simulation | Sim backend: `Julep.Test.EventMap` (pure Elixir inference). Headless/Full: events generated by renderer and sent back via JSONL. |
| Structural snapshots | `snapshot("name")` / `assert_snapshot("name")` -- SHA-256 of serialized tree JSON, works on all backends. |
| Pixel screenshots | `screenshot("name")` / `assert_screenshot("name")` -- SHA-256 of RGBA pixel data, full backend only. No-op on sim/headless. |
| Widget finding | Headless/Full: tree traversal in Rust (`headless.rs`, `test_mode.rs`). Sim: tree traversal in Elixir. |
| Script execution | `Julep.Test.Script.Runner` (Elixir) |

Julep does not link against `iced_test` directly in its current
implementation. The headless backend uses `Core` with JSONL serialization
