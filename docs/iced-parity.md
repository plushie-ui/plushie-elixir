...
| `Simulator::click(id)` | `click("#id")` via scripting protocol |
| `Simulator::find(id)` | `find!("#id")` via query protocol |
| `Simulator::snapshot()` | `tree_hash("name")` structural tree hash via tree_hash protocol |
| -- | `screenshot("name")` pixel screenshot (windowed backend only, no-op on pooled_mock/headless) |
...
| Event simulation | Pooled_mock backend: pure Elixir inference. Headless/Windowed: events generated by renderer and sent back via wire protocol. |
| Structural tree hashes | `tree_hash("name")` / `assert_tree_hash("name")` -- SHA-256 of serialized tree JSON, works on all backends. |
| Pixel screenshots | `screenshot("name")` / `assert_screenshot("name")` -- SHA-256 of RGBA pixel data. Windowed and headless backends produce real screenshots; pooled_mock returns stubs. |
| Widget finding | Headless/Full: tree traversal in Rust (`headless.rs`, `scripting.rs`). Mock: tree traversal in Elixir. |
