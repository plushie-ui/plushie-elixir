...
`window_id` appended as the last element), `window_id` (window).
...
`Command.focus("form/email")`. `Plushie.Event.target/1`
reconstructs the full path from an event when needed; it strips
the trailing `window_id` from the scope before joining.

Auto-ID containers (no explicit ID, ID generated by the DSL) do
not create a scope. Window nodes do not create a scope; they
are the window component of the wire ID. Both `"/"` and `"#"`
are forbidden in user-provided IDs (`/` is the scope separator;
`#` is the window separator).
