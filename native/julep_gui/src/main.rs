...
struct App {
    ...
    stdin_closed: bool,
    /// Bundled widget caches for text_editor, markdown, combo_box, pane_grid.
    caches: WidgetCaches,
    ...
    fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::TextEditorAction(id, action) => {
                ...
                if let Some(content) = self.caches.editor_contents.get_mut(&id) {
            ...
            Message::PaneResized(grid_id, evt) => {
                if let Some(state) = self.caches.pane_grid_states.get_mut(&grid_id) {
            ...
            Message::PaneClicked(grid_id, pane) => {
                if let Some(state) = self.caches.pane_grid_states.get(&grid_id) {
                    let pane_id = state.get(pane).cloned().unwrap_or_default();
    ...
    fn handle_widget_op(&mut self, op: &str, payload: &serde_json::Value) -> Task<Message> {
        ...
        match op {
            ...
            "pane_split" => {
                ...
                if let Some(state) = self.caches.pane_grid_states.get_mut(&target) {
