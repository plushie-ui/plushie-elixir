defmodule Plushie.Test.Backend.CommandProcessor do
  @moduledoc """
  ...
  """
  @spec process(app :: module(), model :: term(), commands :: [Plushie.Command.t()]) :: term()
  ...
  defp do_process(app, model, commands, depth) do
    Enum.reduce(commands, model, fn
      %Plushie.Command{type: :async, payload: %{fun: fun, tag: tag}}, acc ->
        result = fun.()

        {new_model, new_commands} =
          dispatch_update(app, acc, %Plushie.Event.Async{tag: tag, result: result})

        do_process(app, new_model, new_commands, depth + 1)

      %Plushie.Command{type: :stream, payload: %{fun: fun, tag: tag}}, acc ->
        ref = make_ref()
        parent = self()
        ...
        {new_model, new_commands} =
          dispatch_update(app, acc, %Plushie.Event.Async{tag: tag, result: final})

        do_process(app, new_model, new_commands, depth + 1)

      %Plushie.Command{type: :done, payload: %{value: value, mapper: mapper}}, acc ->
        event = mapper.(value)
        {new_model, new_commands} = dispatch_update(app, acc, event)
        do_process(app, new_model, new_commands, depth + 1)

      %Plushie.Command{type: :batch, payload: %{commands: cmds}}, acc ->
        do_process(app, acc, cmds, depth + 1)

      %Plushie.Command{type: :none}, acc ->
        acc
  ...
  defp drain_stream(app, model, tag, ref, depth) do
    receive do
      {:cp_stream, ^ref, value} ->
        {new_model, new_commands} =
          dispatch_update(app, model, %Plushie.Event.Stream{tag: tag, value: value})
  ...
  defp normalize_commands(%Plushie.Command{} = cmd), do: [cmd]
