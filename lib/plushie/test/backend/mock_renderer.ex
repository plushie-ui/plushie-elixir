defmodule Plushie.Test.Backend.MockRenderer do
  ...
  Start a pool in `test_helper.exs`:
      ...
      {:ok, pool} = Plushie.Test.SessionPool.start_link(
        mode: :mock,
  ...
  alias Plushie.Test.Backend.EventDecoder
  ...
  def handle_call({:find, selector}, _from, state) do
    ...
    case SessionPool.send_message(
      ...
      {:ok, %{"data" => data}} when data != nil and data != %{} ->
        {:reply, Plushie.Test.Element.from_node(data), state}
  ...
  def handle_call({:tree_hash, name}, _from, state) do
    case SessionPool.send_message(
      ...
      {:ok, resp} ->
        tree_hash = %Plushie.Test.TreeHash{
          name: resp["name"],
          hash: resp["hash"]
        }
  ...
  def handle_call({:screenshot, name}, _from, state) do
    case SessionPool.send_message(
      ...
      {:ok, resp} ->
        screenshot = %Plushie.Test.Screenshot{
          name: resp["name"],
          hash: resp["hash"] || "",
  ...
  @impl GenServer
  def handle_info(
        {:plushie_pool_event, _session_id, %{"type" => "interact_step", "events" => events}},
        state
      ) do
  ...
  def handle_info(
        {:plushie_pool_event, _session_id, %{"type" => "interact_response", "events" => events}},
        state
      ) do
  ...
  def handle_info({:plushie_pool_event, _session_id, event}, state) do
