defmodule Toddy.Runtime.Windows do
  @moduledoc """
  Window lifecycle management for the Toddy runtime.
  ...
  def sync_windows(state, tree) do
    ...
    Enum.each(closed, fn window_id ->
      if state.bridge do
        Toddy.Bridge.send_window_op(state.bridge, "close", window_id)
      end
    end)
    ...
    Enum.each(surviving, fn window_id ->
      ...
      if old_props != new_props and state.bridge do
        Toddy.Bridge.send_window_op(state.bridge, "update", window_id, new_props)
      end
    end)
