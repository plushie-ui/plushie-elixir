defmodule Toddy.Iced.Theme do
  ...
  ## Examples
      ...
      iex> Toddy.Iced.Theme.custom("Tokyo Remix", primary: "#7aa2f7", danger: "#f7768e")
      %{"name" => "Tokyo Remix", "primary" => "#7aa2f7", "danger" => "#f7768e"}

      iex> Toddy.Iced.Theme.custom("Nord+", base: :nord, primary: "#88c0d0")
  ...
  def custom(name, opts \\ []) when is_binary(name) and is_list(opts) do
    ...
    Enum.reduce(opts, result, fn {key, value}, acc ->
      Map.put(acc, Atom.to_string(key), Toddy.Iced.Color.cast(value))
    end)
  end
