defmodule Toddy.Iced.Widget.Text do
  @moduledoc """
  ...
  def color(%__MODULE__{} = txt, color), do: %{txt | color: Toddy.Iced.Color.cast(color)}
  ...
  @doc "Converts this text struct to a `ui_node()` map via the `Toddy.Iced.Widget` protocol."
  @spec build(text :: t()) :: Toddy.Iced.ui_node()
  def build(%__MODULE__{} = txt), do: Toddy.Iced.Widget.to_node(txt)
