defmodule Toddy.Iced.Widget.Row do
  @moduledoc """
  ...
  - `padding` (number | map) -- padding inside the row. See `Toddy.Iced.Padding`.
  - `width` (length) -- width of the row. Default: shrink. See `Toddy.Iced.Length`.
  - `height` (length) -- height of the row. Default: shrink.
  - `align_y` -- vertical alignment of children: `:top`, `:center`, `:bottom`. See `Toddy.Iced.Alignment`.
  ...
  @spec push(row :: t(), child :: Toddy.Iced.ui_node() | struct()) :: t()
  def push(%__MODULE__{} = row, child), do: %{row | children: [child | row.children]}

  @doc "Appends multiple children to the row."
  @spec extend(row :: t(), children :: [Toddy.Iced.ui_node() | struct()]) :: t()
  def extend(%__MODULE__{} = row, children),
    do: %{row | children: Enum.reverse(children) ++ row.children}

  @doc "Sets accessibility annotations."
  @spec a11y(row :: t(), a11y :: Toddy.Iced.A11y.t()) :: t()
  def a11y(%__MODULE__{} = row, a11y), do: %{row | a11y: A11y.cast(a11y)}

  @doc "Converts this row struct to a `ui_node()` map via the `Toddy.Iced.Widget` protocol."
  @spec build(row :: t()) :: Toddy.Iced.ui_node()
  def build(%__MODULE__{} = row), do: Toddy.Iced.Widget.to_node(row)
