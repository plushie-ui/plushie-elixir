defmodule Toddy.Iced.Widget.RichText do
  @moduledoc """
  ...
  def color(%__MODULE__{} = rt, color), do: %{rt | color: Toddy.Iced.Color.cast(color)}
  ...
  @doc "Converts this rich text struct to a `ui_node()` map via the `Toddy.Iced.Widget` protocol."
  @spec build(rich_text :: t()) :: Toddy.Iced.ui_node()
  def build(%__MODULE__{} = rt), do: Toddy.Iced.Widget.to_node(rt)
