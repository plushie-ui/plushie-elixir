defmodule Toddy.Iced.Widget.Build do
  ...
  def put_if(props, value, key), do: Map.put(props, key, Toddy.Iced.Encode.encode(value))
  ...
  @spec to_node(child :: map() | struct()) :: Toddy.Iced.ui_node()
  def to_node(%{id: _, type: _, props: _, children: _} = node), do: node
  def to_node(%_module{} = widget), do: Toddy.Iced.Widget.to_node(widget)

  @doc "Converts a list of children (maps or structs) to ui_node() maps."
  @spec children_to_nodes(children :: [map() | struct()]) :: [Toddy.Iced.ui_node()]
