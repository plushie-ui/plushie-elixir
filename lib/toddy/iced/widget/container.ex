defmodule Toddy.Iced.Widget.Container do
  @moduledoc """
  ...
  def background(%__MODULE__{} = c, background),
    do: %{c | background: Toddy.Iced.Color.cast(background)}

  @doc "Sets the text color override."
  @spec color(container :: t(), color :: Toddy.Iced.Color.input()) :: t()
  def color(%__MODULE__{} = c, color), do: %{c | color: Toddy.Iced.Color.cast(color)}
  ...
  @doc "Converts this container struct to a `ui_node()` map via the `Toddy.Iced.Widget` protocol."
  @spec build(container :: t()) :: Toddy.Iced.ui_node()
  def build(%__MODULE__{} = c), do: Toddy.Iced.Widget.to_node(c)
