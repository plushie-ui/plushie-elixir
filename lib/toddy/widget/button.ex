defmodule Toddy.Widget.Button do
  @moduledoc """
  ...
  - `style` -- named preset atom (`:primary` (default), `:secondary`, `:success`,
    ...
    for custom styling. See `Toddy.Type.StyleMap`.
  - `width` (length) -- button width. Default: shrink. See `Toddy.Type.Length`.
  ...
  @spec a11y(button :: t(), a11y :: Toddy.Type.A11y.t()) :: t()
  def a11y(%__MODULE__{} = btn, a11y), do: %{btn | a11y: A11y.cast(a11y)}

  @doc "Converts this button struct to a `ui_node()` map via the `Toddy.Widget` protocol."
  @spec build(button :: t()) :: Toddy.Widget.ui_node()
  def build(%__MODULE__{} = btn), do: Toddy.Widget.to_node(btn)
