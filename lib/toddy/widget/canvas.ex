defmodule Toddy.Widget.Canvas do
  @moduledoc """
  ...
  @spec width(canvas :: t(), width :: Toddy.Type.Length.t()) :: t()
  def width(%__MODULE__{} = canvas, width), do: %{canvas | width: width}

  @doc "Sets the canvas height."
  @spec height(canvas :: t(), height :: Toddy.Type.Length.t()) :: t()
  ...
  @spec background(canvas :: t(), background :: Toddy.Type.Color.input()) :: t()
  def background(%__MODULE__{} = canvas, background),
    do: %{canvas | background: Toddy.Type.Color.cast(background)}
  ...
  @doc "Converts this canvas struct to a `ui_node()` map via the `Toddy.Widget` protocol."
  @spec build(canvas :: t()) :: Toddy.Widget.ui_node()
  def build(%__MODULE__{} = canvas), do: Toddy.Widget.to_node(canvas)
