defmodule Toddy.Widget.Row do
  ...
  - `padding` (number | map) -- padding inside the row. See `Toddy.Type.Padding`.
  - `width` (length) -- width of the row. Default: shrink. See `Toddy.Type.Length`.
  - `height` (length) -- height of the row. Default: shrink.
  - `align_y` -- vertical alignment of children: `:top`, `:center`, `:bottom`. See `Toddy.Type.Alignment`.
  ...
  @spec push(row :: t(), child :: Toddy.Widget.ui_node() | struct()) :: t()
  def push(%__MODULE__{} = row, child), do: %{row | children: [child | row.children]}

  @doc "Appends multiple children to the row."
  @spec extend(row :: t(), children :: [Toddy.Widget.ui_node() | struct()]) :: t()
  def extend(%__MODULE__{} = row, children),
    do: %{row | children: Enum.reverse(children) ++ row.children}

  @doc "Sets accessibility annotations."
  @spec a11y(row :: t(), a11y :: Toddy.Type.A11y.t()) :: t()
  def a11y(%__MODULE__{} = row, a11y), do: %{row | a11y: A11y.cast(a11y)}

  @doc "Converts this row struct to a `ui_node()` map via the `Toddy.Widget` protocol."
  @spec build(row :: t()) :: Toddy.Widget.ui_node()
  def build(%__MODULE__{} = row), do: Toddy.Widget.to_node(row)
