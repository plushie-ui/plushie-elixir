defmodule Toddy.Widget.Window do
  @moduledoc """
  ...
  @spec push(window :: t(), child :: Toddy.Widget.ui_node() | struct()) :: t()
  def push(%__MODULE__{} = w, child), do: %{w | children: [child | w.children]}

  @doc "Appends multiple children to the window."
  @spec extend(window :: t(), children :: [Toddy.Widget.ui_node() | struct()]) :: t()
  def extend(%__MODULE__{} = w, children),
    do: %{w | children: Enum.reverse(children) ++ w.children}

  @doc "Converts this window struct to a `ui_node()` map via the `Toddy.Widget` protocol."
  @spec build(window :: t()) :: Toddy.Widget.ui_node()
