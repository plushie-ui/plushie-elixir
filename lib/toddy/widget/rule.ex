defmodule Toddy.Widget.Rule do
  @moduledoc """
  ...
  - `direction` -- `:horizontal` (default) or `:vertical`. See `Toddy.Type.Direction`.
  - `style` -- named preset atom (`:default`, `:weak`) or `StyleMap.t()` for
    custom styling. See `Toddy.Type.StyleMap`.
  ...
  @spec direction(rule :: t(), direction :: Toddy.Type.Direction.t()) :: t()
  ...
  @spec a11y(rule :: t(), a11y :: Toddy.Type.A11y.t()) :: t()
  def a11y(%__MODULE__{} = rule, a11y), do: %{rule | a11y: A11y.cast(a11y)}

  @doc "Converts this rule struct to a `ui_node()` map via the `Toddy.Widget` protocol."
  @spec build(rule :: t()) :: Toddy.Widget.ui_node()
  def build(%__MODULE__{} = rule), do: Toddy.Widget.to_node(rule)
