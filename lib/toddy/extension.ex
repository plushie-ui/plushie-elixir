defmodule Toddy.Extension do
  @moduledoc """
  Macro-based DSL for declaring Toddy widget extensions.
  ...
  defmacro __using__(kind) when kind not in [:native_widget, :widget] do
    raise ArgumentError,
          "Toddy.Extension kind must be :native_widget or :widget, got: #{inspect(kind)}"
  end
  ...
  defmacro __using__(kind) when kind in [:native_widget, :widget] do
    ...
    imports =
      if kind == :native_widget do
        ...
        quote do
          import Toddy.Extension, only: [widget: 1, widget: 2, prop: 2, prop: 3]
          import Toddy.UI
        end
      end
    ...
    before_compile =
      quote do
        @before_compile Toddy.Extension
      end
