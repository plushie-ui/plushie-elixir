defmodule Toddy.Test.Script do
  @moduledoc """
  Parser for `.toddy` test scripts.

  The `.toddy` format is a superset of iced's `.ice` test script format,
  adding Toddy-specific instructions like `assert_text` and `assert_model`.
  ...
  See `Toddy.Test.Script.Runner` for execution.
  """
  ...
  @doc "Parses a .toddy script from a file path."
  ...
  @doc "Parses a .toddy script from a string."
  ...
  defp parse_header(text) do
    ...
    case Map.fetch(fields, "app") do
      {:ok, app_str} ->
        ...
        backend = parse_backend(Map.get(fields, "backend", "pooled_mock"))
  ...
  defp parse_backend("pooled_mock"), do: :pooled_mock
  defp parse_backend("mock"), do: :pooled_mock
  defp parse_backend("headless"), do: :headless
  defp parse_backend("windowed"), do: :windowed
  defp parse_backend(_), do: :pooled_mock
