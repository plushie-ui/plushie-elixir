defmodule Toddy.Test.Helpers do
  @moduledoc """
  Test helper functions imported by `Toddy.Test.Case`.
  ...
  These are automatically imported when using `Toddy.Test.Case`:
  ...
  `Toddy.Test.Session` directly.
  """

  alias Toddy.Test.Backend
  alias Toddy.Test.Element
  alias Toddy.Test.Screenshot
  alias Toddy.Test.Session
  alias Toddy.Test.TreeHash

  @doc "Returns the current test session from the process dictionary."
  @spec session() :: Session.t()
  def session do
    Process.get(:toddy_test_session) ||
      raise "No Toddy test session found. Are you using `use Toddy.Test.Case`?"
  end
  ...
  defmacro assert_text(selector, expected) do
    quote do
      element = Toddy.Test.Helpers.find!(unquote(selector))
  ...
  defmacro assert_exists(selector) do
    quote do
      unless Toddy.Test.Helpers.find(unquote(selector)) do
        raise ExUnit.AssertionError,
  ...
  defmacro assert_not_exists(selector) do
    quote do
      if Toddy.Test.Helpers.find(unquote(selector)) do
        raise ExUnit.AssertionError,
  ...
  defmacro assert_a11y(selector, expected) do
    quote do
      element = Toddy.Test.Helpers.find!(unquote(selector))
      raw_a11y = Toddy.Test.Element.a11y(element)
  ...
  defmacro assert_role(selector, expected_role) do
    quote do
      element = Toddy.Test.Helpers.find!(unquote(selector))
      actual_role = Toddy.Test.Element.inferred_role(element)
  ...
  Set `TODDY_UPDATE_SNAPSHOTS=1` to force-update golden files.
  """
  ...
  On first run, creates the golden file. On subsequent runs, compares hashes.
  Set `TODDY_UPDATE_SCREENSHOTS=1` to force-update golden files. No-op on
  ...
  Resolves the backend from `TODDY_TEST_BACKEND` env var, application
  ...
  Call this in a test or setup block when not using `Toddy.Test.Case`.
  """
  ...
  defmacro assert_model(expected) do
    quote do
      actual = Toddy.Test.Helpers.model()
