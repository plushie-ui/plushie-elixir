defmodule Toddy.Test.Backend.RendererBase do
  @moduledoc """
  ...
  defmacro __using__(opts) do
    quote do
      ...
      # Event decoding is handled by Toddy.Test.Backend.EventDecoder.
      ...
      def handle_info({port, {:data, {:eol, line}}}, %{port: port, format: :json} = state) do
        ...
        case Toddy.Protocol.decode(full_line, :json) do
          {:ok, response} ->
      ...
      def handle_info({port, {:data, binary}}, %{port: port, format: :msgpack} = state)
        ...
        case Toddy.Protocol.decode(binary, :msgpack) do
          {:ok, response} ->
            {:noreply, handle_response(response, state)}
      ...
      defp dispatch_event(%{"family" => type, "id" => id} = event, state) do
        elixir_event = Toddy.Test.Backend.EventDecoder.decode(type, id, event)
