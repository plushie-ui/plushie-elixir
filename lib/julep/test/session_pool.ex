defmodule Julep.Test.SessionPool do
  @moduledoc """
  Shared renderer process for concurrent test sessions.

  Owns a single `julep --headless --max-sessions N` (or `--mock`)
  ...
  Then use `Julep.Test.Backend.Pooled` as the backend, passing the pool:
  ...
  defp forward_to_session(session_id, msg, state) do
    case Map.get(state.sessions, session_id) do
      nil -> state
      pid -> send(pid, {:julep_pool_event, session_id, msg})
    end
