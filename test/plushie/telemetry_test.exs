defmodule Plushie.TelemetryTest do
  ...
  describe "[:plushie, :view] span" do
    test "fires start and stop events on initial render" do
      capture_log(fn ->
        stop_id = attach([:plushie, :view, :stop], self())
  ...
  describe "[:plushie, :update] span" do
  ...
  describe "[:plushie, :bridge, :send]" do
  ...
  describe "[:plushie, :bridge, :receive]" do
    test "handler can be attached for receive events" do
      capture_log(fn ->
        handler_id = attach([:plushie, :bridge, :receive], self())
        :telemetry.detach(handler_id)
      end)
    end
  end

  describe "[:plushie, :bridge, :restart]" do
    test "handler can be attached for restart events" do
      capture_log(fn ->
        handler_id = attach([:plushie, :bridge, :restart], self())
        :telemetry.detach(handler_id)
      end)
    end
  end
