defmodule Plushie.Docs.ScopedIdsTest do
  ...
  test "scoped_ids_match_immediate_parent_test" do
    ...
    assert match?(%WidgetEvent{type: :click, id: "save", scope: ["form" | _]}, event)
  end

  # -- Bind parent for dynamic lists -----------------------------------------

  test "scoped_ids_dynamic_list_bind_parent_test" do
    event = %WidgetEvent{type: :toggle, id: "done", value: true, scope: ["item_42", "todo_list"]}

    assert %WidgetEvent{type: :toggle, id: "done", scope: [item_id | _]} = event
  ...
  test "scoped_ids_dynamic_list_delete_test" do
    event = %WidgetEvent{type: :click, id: "delete", scope: ["item_7", "todo_list"]}

    assert %WidgetEvent{type: :click, id: "delete", scope: [item_id | _]} = event
  ...
  test "scoped_ids_depth_agnostic_test" do
    ...
    assert match?(%WidgetEvent{type: :input, id: "query", scope: ["search" | _]}, event)
  end

  # -- Exact depth matching --------------------------------------------------

  test "scoped_ids_exact_depth_test" do
    event = %WidgetEvent{type: :input, id: "query", value: "hi", scope: ["search"]}

    assert match?(%WidgetEvent{type: :input, id: "query", scope: ["search"]}, event)
  end

  test "scoped_ids_exact_depth_mismatch_test" do
    # Two scope levels should NOT match the exact single-scope pattern
    event = %WidgetEvent{type: :input, id: "query", value: "hi", scope: ["search", "panel"]}

    refute match?(%WidgetEvent{type: :input, id: "query", scope: ["search"]}, event)
  end
  ...
  test "scoped_ids_no_scope_test" do
    ...
    assert match?(%WidgetEvent{type: :click, id: "save", scope: []}, event)
  end
