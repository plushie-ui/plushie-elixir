defmodule Plushie.Type.PaddingTest do
  ...
  describe "cast/1" do
    test "uniform number produces four equal sides" do
      assert Padding.cast(8) == {:ok, %{top: 8, right: 8, bottom: 8, left: 8}}
    end

    test "zero padding" do
      assert Padding.cast(0) == {:ok, %{top: 0, right: 0, bottom: 0, left: 0}}
    end

    test "float padding" do
      assert Padding.cast(4.5) == {:ok, %{top: 4.5, right: 4.5, bottom: 4.5, left: 4.5}}
    end

    test "{vertical, horizontal} tuple" do
      assert Padding.cast({4, 12}) == {:ok, %{top: 4, right: 12, bottom: 4, left: 12}}
    end
