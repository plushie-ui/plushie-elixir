defmodule Plushie.PaneGridTest do
  ...
  describe "pane_grid widget construction" do
    test "creates pane_grid node with children" do
      left = Plushie.Widget.Node.build("left", "text", %{content: "Left"})
      right = Plushie.Widget.Node.build("right", "text", %{content: "Right"})
      node = Plushie.Widget.Node.build("pg1", "pane_grid", %{spacing: 4}, [left, right])
      assert node.id == "pg1"
    ...
    test "creates pane_grid with default props" do
      node = Plushie.Widget.Node.build("pg1", "pane_grid", %{})
      assert node.props == %{}
