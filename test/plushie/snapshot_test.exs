defmodule Plushie.SnapshotTest do
  use ExUnit.Case, async: false

  # async: false because we manipulate PLUSHIE_UPDATE_SNAPSHOTS env var
  # and write to the filesystem; parallel runs would stomp on each other.

  import Plushie.UI, only: [button: 2, button: 3, text_input: 3, checkbox: 3]
  ...
  describe "first run (no existing snapshot)" do
    test "creates the snapshot file and returns :ok", ctx do
      ...
      result = Plushie.Test.assert_tree_snapshot(simple_tree(), path)
    ...
    test "creates parent directories if they don't exist", ctx do
      ...
      assert :ok = Plushie.Test.assert_tree_snapshot(simple_tree(), path)
      assert File.exists?(path)
    end
  end
  ...
  describe "PLUSHIE_UPDATE_SNAPSHOTS=1" do
    ...
    test "does not overwrite when env var is set to something other than 1", ctx do
      ...
      System.put_env("PLUSHIE_UPDATE_SNAPSHOTS", "yes")
  ...
  describe "snapshot file format" do
    ...
    test "output is deterministic across multiple writes", ctx do
      ...
      assert :ok = Plushie.Test.assert_tree_snapshot(tree, path_a)
      assert :ok = Plushie.Test.assert_tree_snapshot(tree, path_b)
