defmodule Toddy.RuntimeTest do
  ...
  defmodule CommandApp do
    ...
    def init(_opts) do
      {%{value: 0}, Toddy.Command.send_after(10, :timer_tick)}
    end
  ...
  defmodule AsyncApp do
    ...
    def update(model, %Widget{type: :click, id: "async"}) do
      cmd = Toddy.Command.async(fn -> 42 end, :async_result)
      {model, cmd}
    end

    def update(model, %Toddy.Event.Async{tag: :async_result, result: result}),
  ...
  defp start_runtime(app, extra_opts \\ []) do
    ...
    {:ok, runtime} = Toddy.Runtime.start_link(opts)
  ...
  describe "init/update/view cycle" do
    test "Runtime starts and sends initial snapshot to bridge" do
      ...
      snapshots = Toddy.Test.MockBridge.get_snapshots(bridge)
    ...
    test "dispatching an event updates the model and sends a patch" do
      ...
      patches = Toddy.Test.MockBridge.get_patches(bridge)
    ...
    test "batch commands are all executed" do
      ...
      defmodule BatchApp do
        ...
        def update(model, %Widget{type: :click, id: "batch"}) do
          cmd =
            Toddy.Command.batch([
              Toddy.Command.send_after(10, :tick_a),
              Toddy.Command.send_after(10, :tick_b)
            ])
  ...
  describe "renderer lifecycle" do
    ...
    test ":renderer_restarted re-sends the current tree to the bridge" do
      capture_log(fn ->
        ...
        initial_snapshots = Toddy.Test.MockBridge.get_snapshots(bridge)
  ...
  describe "subscriptions" do
    test "app with every subscription receives periodic tick events" do
      defmodule TickApp do
        ...
        def subscribe(_model), do: [Toddy.Subscription.every(20, :tick)]
    ...
    test "multiple subscriptions can coexist" do
      defmodule MultiSubApp do
        ...
        def update(model, %Toddy.Event.Timer{tag: :fast_tick}),
          do: %{model | fast: model.fast + 1}

        def update(model, %Toddy.Event.Timer{tag: :slow_tick}),
        ...
        def subscribe(_model) do
          [
            Toddy.Subscription.every(15, :fast_tick),
            Toddy.Subscription.every(40, :slow_tick)
          ]
        end
  ...
  describe "effects" do
    test "effect command is forwarded to the bridge" do
      ...
      effects = Toddy.Test.MockBridge.get_effects(bridge)
  ...
  describe "subscription registration" do
      ...
      defmodule KeySubApp do
        ...
        def subscribe(model) do
          if model.listening do
            [Toddy.Subscription.on_key_press(:keys)]
          else
            []
          end
        end
    ...
    test "removing a renderer subscription sends unregister message to bridge" do
      ...
      unregisters = Toddy.Test.MockBridge.get_unsubscribes(bridge)
  ...
  describe "multi-window support" do
    test "window nodes in the tree trigger window open ops" do
      ...
      window_ops = Toddy.Test.MockBridge.get_window_ops(bridge)
  ...
  describe "stream and cancel commands" do
    test "stream emits multiple intermediate results through update" do
      defmodule StreamApp do
        ...
        def update(model, %Toddy.Event.Stream{tag: :import, value: {:chunk, value}}) do
          %{model | chunks: model.chunks ++ [value]}
        end

        def update(model, %Toddy.Event.Async{tag: :import, result: :done}) do
          %{model | final: :done}
        end
    ...
    test "cancel stops a running stream task" do
      defmodule CancelStreamApp do
        ...
        def update(model, %Widget{type: :click, id: "cancel"}) do
          {%{model | cancelled: true}, Toddy.Command.cancel(:import)}
        end
    ...
    test "cancel is a no-op when no task is running for the tag" do
      defmodule CancelNoopApp do
        ...
        def update(model, %Widget{type: :click, id: "cancel"}) do
          {%{model | value: model.value + 1}, Toddy.Command.cancel(:nonexistent)}
        end
  ...
  describe "extension config in settings" do
    test "settings includes extension_config from application env" do
      Application.put_env(:toddy, :extension_config, %{"terminal" => %{"shell" => "/bin/bash"}})

      {runtime, bridge} = start_runtime(SimpleApp)
      await_initial_render(runtime)

      settings_list = Toddy.Test.MockBridge.get_settings(bridge)
