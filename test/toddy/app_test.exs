defmodule Toddy.AppTest do
  use ExUnit.Case, async: true

  # ---------------------------------------------------------------------------
  # Inline test module that uses the Toddy.App behaviour with only the
  # required callbacks -- subscribe/1 and handle_renderer_exit/2 are left to
  # the defaults injected by `use Toddy.App`.
  # ---------------------------------------------------------------------------
  ...
  describe "use Toddy.App -- behaviour injection" do
    test "module declares the Toddy.App behaviour" do
      assert Toddy.App in (MinimalApp.module_info(:attributes)[:behaviour] || [])
    end
