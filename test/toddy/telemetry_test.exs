defmodule Toddy.TelemetryTest do
  ...
  describe "[:toddy, :view] span" do
    test "fires start and stop events on initial render" do
      capture_log(fn ->
        stop_id = attach([:toddy, :view, :stop], self())

        {runtime, _bridge} = start_runtime(TelemetryApp)
        await_initial_render(runtime)

        assert_receive {:telemetry_event, [:toddy, :view, :stop], measurements, _metadata}
  ...
  describe "[:toddy, :update] span" do
    test "fires start and stop events when dispatching an event" do
      capture_log(fn ->
        {runtime, _bridge} = start_runtime(TelemetryApp)
        await_initial_render(runtime)

        stop_id = attach([:toddy, :update, :stop], self())

        Toddy.Runtime.dispatch(runtime, %Widget{type: :click, id: "inc"})
        # Synchronise -- ensures the event has been processed.
        :sys.get_state(runtime)

        assert_receive {:telemetry_event, [:toddy, :update, :stop], measurements, _metadata}
        assert is_integer(measurements.duration)
        assert measurements.duration >= 0
  ...
  describe "[:toddy, :bridge, :send]" do
    test "fires when sending data to the port" do
      capture_log(fn ->
        handler_id = attach([:toddy, :bridge, :send], self())
  ...
  describe "[:toddy, :bridge, :receive]" do
    test "handler can be attached for receive events" do
      capture_log(fn ->
        handler_id = attach([:toddy, :bridge, :receive], self())
        :telemetry.detach(handler_id)
      end)
    end
  end

  describe "[:toddy, :bridge, :restart]" do
    test "handler can be attached for restart events" do
      capture_log(fn ->
        handler_id = attach([:toddy, :bridge, :restart], self())
        :telemetry.detach(handler_id)
      end)
    end
  end
