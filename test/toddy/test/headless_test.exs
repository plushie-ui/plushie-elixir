defmodule Toddy.Test.HeadlessTest do
  @moduledoc """
  Tests for the Headless test backend.

  Requires `cargo build` (all modes are compiled unconditionally).
  Enabled by setting TODDY_HEADLESS=1 or TODDY_TEST_BACKEND=headless.
  """
  ...
  unless System.get_env("TODDY_HEADLESS") || System.get_env("TODDY_TEST_BACKEND") == "headless" do
    @moduletag :skip
  end

  # These tests are run via the standard test suite with TODDY_TEST_BACKEND=headless.
  ...
  test "placeholder - headless backend starts" do
    # Covered by running the full suite with TODDY_TEST_BACKEND=headless
  end
end
