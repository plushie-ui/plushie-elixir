defmodule ToddyUITestHelper do
  @moduledoc false
  import Toddy.UI
...
defmodule Toddy.UITest do
  ...
  describe "text/1" do
    ...
    test "auto-generates an id containing the module name" do
      ...
      assert String.contains?(node.id, "Toddy.UITest")
    end
  ...
  describe "column do...end" do
    test "collects children from do block" do
      node = ToddyUITestHelper.simple_column()
      assert node.type == "column"
  ...
  describe "find/2" do
    test "finds a node by id in the tree" do
      ...
      result = Toddy.UI.find(tree, "save")
      assert result != nil
    ...
    test "returns nil when id is not in the tree" do
      tree = window("app")
      assert Toddy.UI.find(tree, "ghost") == nil
    end
  end
