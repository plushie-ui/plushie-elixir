defmodule Toddy.BinaryTest do
  ...
  describe "binary_name/0" do
    ...
    test "starts with 'toddy-'" do
      assert String.starts_with?(Binary.binary_name(), "toddy-")
    end
  ...
  describe "renderer_path/0 with TODDY_RENDERER_PATH set" do
    setup do
      # Create a temporary file to act as the binary
      tmp_dir = System.tmp_dir!()
      fake_bin = Path.join(tmp_dir, "toddy_renderer_fake_#{System.unique_integer([:positive])}")
      File.write!(fake_bin, "#!/bin/sh\n")
      File.chmod!(fake_bin, 0o755)

      previous = System.get_env("TODDY_RENDERER_PATH")
      System.put_env("TODDY_RENDERER_PATH", fake_bin)

      on_exit(fn ->
        if previous,
          do: System.put_env("TODDY_RENDERER_PATH", previous),
          else: System.delete_env("TODDY_RENDERER_PATH")
    ...
    test "returns the env var path when TODDY_RENDERER_PATH is set", %{fake_bin: fake_bin} do
  ...
  describe "renderer_path/0 without any valid path" do
    setup do
      previous = System.get_env("TODDY_RENDERER_PATH")
      System.delete_env("TODDY_RENDERER_PATH")

      on_exit(fn ->
        if previous, do: System.put_env("TODDY_RENDERER_PATH", previous)
      end)
    end
    ...
    test "raises when no binary can be found and env var is not set" do
      ...
      try do
        ...
        e in RuntimeError ->
          assert Exception.message(e) =~ "toddy binary not found"
      end
    end
  end
end
