defmodule Toddy.RuntimeRerenderTest do
  ...
  defp start_runtime(app) do
    ...
    {:ok, runtime} = Toddy.Runtime.start_link(app: app, bridge: bridge_name, name: runtime_name)
  ...
  describe "force_rerender" do
    test "re-renders view with existing model, no update/2 call" do
      capture_log(fn ->
        ...
        patches_before = length(Toddy.Test.MockBridge.get_patches(bridge))
        ...
        patches_after = length(Toddy.Test.MockBridge.get_patches(bridge))
