defmodule Toddy.SnapshotTest do
  use ExUnit.Case, async: false

  # async: false because we manipulate TODDY_UPDATE_SNAPSHOTS env var
  # and write to the filesystem; parallel runs would stomp on each other.

  import Toddy.UI, only: [button: 2, button: 3, text_input: 3, checkbox: 3]
  ...
  setup do
    ...
    System.delete_env("TODDY_UPDATE_SNAPSHOTS")
    :ok
  end
  ...
  describe "first run (no existing snapshot)" do
    test "creates the snapshot file and returns :ok", ctx do
      ...
      result = Toddy.Test.assert_tree_snapshot(simple_tree(), path)
    ...
    test "creates parent directories if they don't exist", ctx do
      ...
      assert :ok = Toddy.Test.assert_tree_snapshot(simple_tree(), path)
      assert File.exists?(path)
    end
  end
  ...
  describe "matching snapshot" do
    test "returns :ok when tree matches stored snapshot", ctx do
      ...
      # First run -- writes
      assert :ok = Toddy.Test.assert_tree_snapshot(tree, path)
      # Second run -- compares
      assert :ok = Toddy.Test.assert_tree_snapshot(tree, path)
    end
  ...
  describe "mismatched snapshot" do
    test "raises ExUnit.AssertionError on mismatch", ctx do
      ...
      # Write the original
      assert :ok = Toddy.Test.assert_tree_snapshot(original, path)

      # Compare with the changed tree -- should blow up
      error =
        assert_raise ExUnit.AssertionError, fn ->
          Toddy.Test.assert_tree_snapshot(changed, path)
        end

      assert error.message =~ "Snapshot mismatch"
      assert error.message =~ path
      assert error.message =~ "TODDY_UPDATE_SNAPSHOTS=1"
    end
  ...
  # ---------------------------------------------------------------------------
  # 4. TODDY_UPDATE_SNAPSHOTS=1 overwrites existing snapshots
  # ---------------------------------------------------------------------------

  describe "TODDY_UPDATE_SNAPSHOTS=1" do
    test "overwrites an existing snapshot instead of failing", ctx do
      ...
      # Enable update mode
      System.put_env("TODDY_UPDATE_SNAPSHOTS", "1")

      # Should not raise -- should overwrite
      assert :ok = Toddy.Test.assert_tree_snapshot(changed, path)
    ...
    test "does not overwrite when env var is set to something other than 1", ctx do
      ...
      System.put_env("TODDY_UPDATE_SNAPSHOTS", "yes")
  ...
  describe "snapshot file format" do
    ...
    test "output is deterministic across multiple writes", ctx do
      ...
      assert :ok = Toddy.Test.assert_tree_snapshot(tree, path_a)
      assert :ok = Toddy.Test.assert_tree_snapshot(tree, path_b)
