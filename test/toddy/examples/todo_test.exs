defmodule TodoTest do
  ...
  describe "adding todos" do
    ...
    test "submit returns focus command" do
      ...
      assert %Toddy.Command{type: :focus} = cmd
    end
  ...
  describe "view/1" do
    test "contains expected structure" do
      model = Todo.init([])
      tree = Toddy.Tree.normalize(Todo.view(model))

      assert Toddy.Tree.exists?(tree, "main")
      assert Toddy.Tree.find(tree, "title")
      assert Toddy.Tree.find(tree, "new_todo")
      assert Toddy.Tree.find(tree, "filter_all")
      assert Toddy.Tree.find(tree, "filter_active")
      assert Toddy.Tree.find(tree, "filter_done")
    end
    ...
    test "renders todo items with scoped IDs" do
      ...
      assert Toddy.Tree.find(tree, "app/list/todo_1/toggle")
      assert Toddy.Tree.find(tree, "app/list/todo_1/delete")
    end
    ...
    test "filters todos in view" do
      ...
      assert Toddy.Tree.exists?(tree, "app/list/todo_2/toggle")
      refute Toddy.Tree.exists?(tree, "app/list/todo_1/toggle")
    end
  end
