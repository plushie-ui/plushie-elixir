defmodule NotesTest do
  ...
  describe "init/1" do
    test "starts with empty notes and list route" do
      model = Notes.init([])
      assert Toddy.State.get(model.state, [:notes]) == []
      assert Toddy.Route.current(model.route) == "/list"
    end
    ...
    test "starts with empty undo state" do
      model = Notes.init([])
      assert Toddy.Undo.current(model.undo) == %{text: "", title: ""}
    end
  end
  ...
  describe "note management" do
    test "creating a new note" do
      ...
      notes = Toddy.State.get(model.state, [:notes])
      assert length(notes) == 1
      assert Toddy.Route.current(model.route) == "/edit"
    end
    ...
    test "editing a note title" do
      ...
      assert Toddy.Undo.current(model.undo).title == "My Note"
    end
    ...
    test "editing a note body" do
      ...
      assert Toddy.Undo.current(model.undo).text == "Some content"
    end
    ...
    test "edits are saved when navigating back" do
      ...
      [note] = Toddy.State.get(model.state, [:notes])
    ...
    test "undo/redo" do
      ...
      assert Toddy.Undo.current(model.undo).title == "First"

      model = Notes.update(model, %Widget{type: :click, id: "redo"})
      assert Toddy.Undo.current(model.undo).title == "Second"
    end
    ...
    test "undo all the way back to initial state" do
      ...
      model = Notes.update(model, %Widget{type: :click, id: "undo"})
      assert Toddy.Undo.current(model.undo).title == ""
    end
    ...
    test "delete clears selection afterwards" do
      ...
      assert Toddy.Selection.selected(model.selection) == MapSet.new()
    end
    ...
    test "clicking a note loads its content into undo" do
      ...
      assert Toddy.Undo.current(model.undo).title == "Hello"
      assert Toddy.Undo.current(model.undo).text == "World"
      # Fresh undo stack -- no history from previous edit session
      refute Toddy.Undo.can_undo?(model.undo)
    end
    ...
    test "search filters notes" do
      ...
      assert Toddy.State.get(model.state, [:search_query]) == "shop"
    end
  end
  ...
  describe "selection" do
    test "toggling note selection on" do
      ...
      model = Notes.update(model, %Widget{type: :toggle, id: "note_select:1", value: true})
      assert Toddy.Selection.selected?(model.selection, 1)
    end
    ...
    test "toggling note selection off" do
      ...
      refute Toddy.Selection.selected?(model.selection, 1)
    end
    ...
    test "multi-select allows selecting multiple notes" do
      ...
      assert Toddy.Selection.selected?(model.selection, 1)
      assert Toddy.Selection.selected?(model.selection, 2)
    end
  end
  ...
  describe "view/1" do
    ...
    test "list view contains expected widgets" do
      ...
      tree = Notes.view(model)
      assert Toddy.UI.exists?(tree, "search")
      assert Toddy.UI.exists?(tree, "new_note")
      assert Toddy.UI.exists?(tree, "delete_selected")
      assert Toddy.UI.exists?(tree, "note:1")
    end
    ...
    test "edit view contains expected widgets" do
      ...
      tree = Notes.view(model)
      assert Toddy.UI.exists?(tree, "back")
      assert Toddy.UI.exists?(tree, "undo")
      assert Toddy.UI.exists?(tree, "redo")
      assert Toddy.UI.exists?(tree, "title")
      assert Toddy.UI.exists?(tree, "body")
    end
  end
  ...
  describe "data query integration" do
    test "Data.query filters notes by title" do
      ...
      result = Toddy.Data.query(notes, search: {[:title, :body], "shop"})
    ...
    test "Data.query filters notes by body" do
      ...
      result = Toddy.Data.query(notes, search: {[:title, :body], "budget"})
      assert length(result.entries) == 1
