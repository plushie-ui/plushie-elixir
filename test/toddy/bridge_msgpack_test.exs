defmodule Toddy.BridgeMsgpackTest do
  @moduledoc """
  ...
  defp test_env do
    Toddy.RendererEnv.build(rust_log: "off")
  end
  ...
  describe "msgpack protocol integration" do
    ...
    test "renderer decodes msgpack settings without protocol error" do
      ...
      settings = Toddy.Protocol.encode_settings(%{antialiasing: false}, :msgpack)
  ...
  describe "bridge telemetry" do
    ...
    test "send and receive events fire during msgpack exchange" do
      capture_log(fn ->
        ...
        Toddy.Bridge.send_settings(bridge, %{antialiasing: false})

        # Give the renderer time to respond.
        Process.sleep(200)

        # The send event should have fired.
        assert_received {:tel, [:toddy, :bridge, :send], %{byte_size: size}}
  ...
  describe "msgpack edge cases" do
    ...
    test "binary data round-trips through msgpack encoding" do
      ...
      encoded = Toddy.Protocol.encode(msg, :msgpack)

      {:ok, decoded} = Toddy.Protocol.decode(encoded, :msgpack)
