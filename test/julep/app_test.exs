defmodule Julep.AppTest do
  use ExUnit.Case, async: true

  # ---------------------------------------------------------------------------
  # Inline test module that uses the Julep.App behaviour with only the
  # required callbacks -- subscribe/1 and handle_renderer_exit/2 are left to
  # the defaults injected by `use Julep.App`.
  # ---------------------------------------------------------------------------
  ...
  describe "use Julep.App -- behaviour injection" do
    test "module declares the Julep.App behaviour" do
      assert Julep.App in (MinimalApp.module_info(:attributes)[:behaviour] || [])
    end
